...
export class CreateOrder {
  ...
  async execute(dto: CreateOrderDTO): Promise<Order> {
    ...
    if (!restaurant.isOpen) {
      throw new Error("Le restaurant est actuellement fermé. Veuillez réessayer plus tard.");
    }

    const items: Consumable[] = await this.consumableRepository.findByIds(dto.itemIds);
    ...
    items.forEach((item: Consumable) => {
      if (item.restaurantId !== dto.restaurantId) {
        throw new Error(`L'item ${item.name} ne provient pas du restaurant sélectionné.`);
      }
      if (!item.isAvailable()) {
